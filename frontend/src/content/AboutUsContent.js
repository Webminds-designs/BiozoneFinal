// aboutUsContent.js
const aboutUsContent = {
	en: {
		topic: "About Us",
		title1: "Where",
		description1:
			"Now through this website, he is ready to guide you towards your academic goals with personalized support and resources.",

		title2: "Biology and",
		description2: "Biology and",

		title3: "Technology",
		description3:
			"His dedication and expertise have made him a trusted mentor for Advanced Level students.",

		title4: "Meet",
		description4:
			"Graduating from the University of Peradeniya, Dr. Charitha Munasinghe has over 10 years of experience in teaching biology and helping students gain admission to Sri Lanka's top state universities.",

		title5: "",
		description5:
			"With his deep knowledge of biology and passion for teaching, he inspires many students to succeed.",
		description6:
			"This website is here to help you achieve your biology goals by offering personalized guidance and essential resources for your learning journey.",
	},
	si: {
		topic: "අපි කවුද",
		title1: "තැන",
		description1:
			"දැන් මෙම වෙබ් අඩවිය හරහා, ඔහු ඔබේ අධ්‍යාපනික ඉලක්ක කරා මාර්ගෝපදේශනය කිරීමට පුද්ගලික සහාය සහ සම්පත් සමඟ සූදානම් වේ.",

		title2: "ජීව විද්‍යාව සහ",
		description2: "ජීව විද්‍යාව ",

		title3: " තාක්ෂණය",
		description3:
			"ඔහුගේ කැපවීම සහ විශේෂඥතාවය ඔහුව උසස් පෙළ සිසුන් සඳහා විශ්වාසනීය උපදේශකයෙකු කර ඇත.",

		title4: "හමුවන",
		description4:
			"පේරාදෙණිය විශ්වවිද්‍යාලයෙන් උපාධිය ලබා ඇති වෛද්‍ය චරිත මුණසිංහ, ජීව විද්‍යාව ඉගැන්වීමේ සහ ශ්‍රී ලංකාවේ ප්‍රමුඛ රාජ්‍ය විශ්වවිද්‍යාලවලට සිසුන් ඇතුළත් කිරීමට උදව් කිරීමේ වසර 10 කට වැඩි පළපුරුද්දක් ඇත.",
		title5: "",
		description5:
			"ජීව විද්‍යාව පිළිබඳ ගැඹුරු අවබෝධයක් සහ ඉගැන්වීමේ ආශාවක් ඇති ඔහු, අධ්‍යයන සාර්ථකත්වය ළඟා කර ගැනීම සඳහා අසංඛ්‍යාත සිසුන්ට දිරිගන්වයි.",
	},
};

export default aboutUsContent;
